...
export const generateInvoiceBuffer = async (order) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Shop name + tagline
            doc.font("Helvetica-Bold").fontSize(21).fillColor(C.dark)
                .text(SHOP.name, M + 14, 20);
            doc.font("Helvetica").fontSize(8).fillColor(C.amber)
                .text("Unique Gifts for Every Occasion", M + 14, 46, { characterSpacing: 0.6 });
            ...
            doc.font("Helvetica-Bold").fontSize(7.5).fillColor(C.zinc)
                .text("Invoice No:", boxX + 10, 60);
            doc.font("Helvetica").fontSize(7.5).fillColor(C.dark)
                .text(invoiceNo, boxX + 78, 60);
            doc.font("Helvetica-Bold").fontSize(7.5).fillColor(C.zinc)
                .text("Date:", boxX + 10, 74);
            doc.font("Helvetica").fontSize(7.5).fillColor(C.dark)
                .text(dt(order.createdAt), boxX + 78, 74);
            doc.font("Helvetica-Bold").fontSize(7.5).fillColor(C.zinc)
                .text("Website:", boxX + 10, 88);
            doc.font("Helvetica").fontSize(7.5).fillColor(C.amber)
                .text(SHOP.website, boxX + 78, 88);
            ...
            drows.forEach(([l, v]) => {
                doc.font("Helvetica-Bold").fontSize(7.5).fillColor(C.mid)
                    .text(l + ":", rightCol, ry, { width: 92 });
                doc.font("Helvetica").fontSize(7.5).fillColor(C.dark)
                    .text(v, rightCol + 96, ry, { width: half - 96 });
                ry += 14;
            });
            ...
            const CW2 = { no: 20, desc: 210, hsn: 56, qty: 44, rate: 56, gst: 40, total: 55 };
            ...
            if (totalGST > 0) {
                doc.font("Helvetica").fontSize(7).fillColor(C.light)
                    .text(
                        "* Tax breakdown: CGST " + rs(totalGST / 2) + "  +  SGST " + rs(totalGST / 2) + "  =  Total GST " + rs(totalGST),
                        M, y, { width: CW },
                    );
                y += 14;
            }
            ...
            doc.font("Helvetica-Bold").fontSize(7.5).fillColor(C.zinc)
                .text("Invoice No: ", M + 96, y + 44);
            doc.font("Helvetica").fontSize(7.5).fillColor(C.amber)
                .text(invoiceNo, M + 158, y + 44);
